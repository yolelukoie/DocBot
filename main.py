...
def handle_start(chat_id: int):
    text = (
        "Привет, дорогая!🌸\n\n"
        "Я бот для отправки и приёма подписанных документов. В комьюнити Femina мы принимаем всех, поэтому нам важно обеспечить информационную безопасность и комфорт для всех участниц. Что дальше делаем?\n\n"
        "1. Я пришлю вам шаблон соглашения.\n"
        "2. Ты его скачаешь себе.\n"
        "3. Можно подписать документ электронной подписью или распечатать и вручную вписать.\n"
        "4. Сохранить подписанный вариант в формате PDF по форме Соглашение_Имя_Фамилия.\n"
        "5. Отправить мне сюда готовый вариант 👌🏻\n\n"
        "Когда будешь отправлять подписанный файл, в подписи к файлу важно указать свои Имя и Фамилию.\n"
        "‼️ Важно: бот принимает только файлы в формате PDF. Подпись нужна на каждой странице!\n\n"
        "Для заполнения документа онлайн можно использовать "
        "<a href=\"https://docs.google.com/forms/d/ТВОЙ_ID\">этот бесплатный ресурс</a>."
    )
...
def handle_send_template(chat_id: int):
    telegram_send_message(
        ...
        "После подписи пришли его мне обратно в формате PDF (с подписью на каждой странице)\n\n"
        "Не забудь указать свою имя и фамилию в подписи к файлу 😉",
    )
...
def handle_update(update: dict):
    ...
    if "text" in message:
        ...
        if text.startswith("/start"):
            ...
            telegram_send_message(
                chat_id,
                "Чтобы получить шаблон соглашения, отправь /document.\n"
